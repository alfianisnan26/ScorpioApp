...
void fetchServerState(var server) async {
  ...
  while (clientState) {
    ...
    if (ret.body == "null" || ret.statusCode != 200) {
      ...
      clientState = false;
      Navigator.pushReplacementNamed(md.currentcontext, '/');
      break;
    }
    if (ret.statusCode == 200) {
      ...
      if (resp.state != serverstate) {
        ...
        if (resp.state == true) {
          Navigator.pushNamed(md.currentcontext,'/ColorScreen');
...
Future<Post> fetchPost(var server) async {
  ...
  if (response.statusCode == 200) {
    ...
    if (ret.dateS == DateTime.now().day) {
      ...
      globalret = ret;
      usertype = ret.type;
      Navigator.pushReplacementNamed(md.currentcontext,'/WaitScreen');
    }else return ret;
  } else {
    return null;
  }
        ...
        child: Column(
          ...
          Row(mainAxisAlignment: MainAxisAlignment.center, children: <Widget>[
            ...
            RaisedButton(
                ...
                onPressed: () {
                  (serverstate == false)?Navigator.pushNamed(context,'/LockScreen')
                  :Navigator.pushNamed(context, '/ColorScreen');
                },
...
void _showAlertDialog(var context) {
        ...
        builder: (context) {
          return AlertDialog(
            ...
            actions: <Widget>[
              ...
              FlatButton(
                onPressed: () {
                  Navigator.popAndPushNamed(context, '/ColorScreen');
                },
  ...
  void uIDdispose(int uid, int server){
